...
DWORD WINAPI sort_characters( void *arg )
{
    int First;
    int Second;
    char TempCh;
...
int main( void )
{
    ...
    HANDLE hThread[MAX_ROW_COUNT];
    int RowNumber[MAX_ROW_COUNT];

    clock_t BeforeClock, AfterClock;
    double  ElapsedSec;

    srand( (unsigned int)time( 0 ) );
    InitializeCriticalSection( &g_CS_Print );
    ...
    for( RowCount = 0; RowCount < MAX_ROW_COUNT; RowCount++ )
    {
        ...
        hThread[RowCount] = CreateThread( NULL, 0, sort_characters, (void *)&RowNumber[RowCount], 0, NULL );
    }
    WaitForMultipleObjects( RowCount, hThread, TRUE, INFINITE );
    AfterClock = clock( );
    ...
    DeleteCriticalSection( &g_CS_Print );
