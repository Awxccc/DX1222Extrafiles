...
{ // ClientSocket is socket descriptor for new incomer.
	int i;
...
// send info of new incomer to previous connected clients.
{
	int i;
	char InfoMessage[100];
	int InfoMessageLength;
		...
		else
		{
			for (Index = 0; Index < TempFds.fd_count; Index++)
			{
				...
				{ // New connection requested by new client.
					...
					send_welcome_message(ClientSocket); // welcome message from server to this connected client.
					session_info_message(ReadFds, ClientSocket); // send previous connected clients info to this new connected client.
					send_notice_message(ReadFds, ClientSocket); // send this new connected client info to all previous connected clients.
				}
				else
